...
export async function getServerSideProps() {
  const library = [];   
  const slateAPI = 'https://slate.host/api/v2/get';
  const response = await fetch(slateAPI, {
    method: 'GET',
    headers: {
  ...
  if (!data) {
    console.log("NO RESPONSE FROM SLATE API!!!")
  }
...
export default function Home({ data }) {
  return (
    <div className="container">
      <Head>
        <title>liveTube App</title>
        <link rel="icon" href="/favicon.ico" />
      </Head>

      <main>
        <h1 className="title">
        <img  src="/livepeer_logo.png"/>
        </h1>
